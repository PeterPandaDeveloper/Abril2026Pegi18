...
func _process(delta):
	# Verificamos que el nodo BarraSalida exista para no tener errores
	if has_node("BarraSalida"):
		if Input.is_key_pressed(KEY_S):
			hold_s += delta
			# Llenamos la barra (el valor va de 0 a 100)
			$BarraSalida.value = (hold_s / 0.8) * 100
			
			if hold_s >= 0.8:
				get_tree().change_scene_to_file("res://BarScene.tscn")
		else:
			# Si sueltas la S, la barra se vacía y el contador se reinicia
			hold_s = 0.0
			$BarraSalida.value = 0	
